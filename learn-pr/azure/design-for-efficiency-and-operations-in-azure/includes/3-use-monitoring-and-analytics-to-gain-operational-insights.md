...
Log Analytics 内のこのデータでは、トラブルシューティング、根本原因の識別、監査のために、生データを照会できます。 (SQL Server、Windows Server Active Directory)、いくつかのことによく知られたサービス、監視データを視覚化するをベスト プラクティスに準拠を発見し、すぐに使用できる管理ソリューションがあります。
...
ログ データすべての Azure とオンプレミス リソースからに送ります Log Analytics ワークスペースでは、イベントの相関関係のログ ソース全体を検索する機能があり、SQL Server および Windows Server Active Directory の管理ソリューションを使用しています。
