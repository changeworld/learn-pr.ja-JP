...
サービスとインフラストラクチャの展開を自動化するには、命令型と宣言型の 2 種類の方法を利用できます。 命令型の方法では、想定する結果を生成するために実行されるコマンドを明示的に示します。 宣言型の方法では、実行方法を指定する代わりに、望ましい結果を指定します。 どちらの方法にも価値があるので、どちらを選んでも誤ることはありません。 これらの異なる方法は Azure ではどのようになっていて、どのように使用するのでしょうか。
...
テンプレートを使用して、Azure のすべてのサービスを作成および操作できます。 テンプレートは、コード リポジトリに格納してソース管理でき、異なる環境間で共有して、開発対象のインフラストラクチャが運用環境内に実際に存在するものと一致することを保証できます。 展開を自動化して一貫性を保証する優れた方法であり、展開が誤って構成されることがなくなり、運用の速度を上げることができます。
...
リソースの展開を自動化することは、環境に対して大きなメリットになる場合があります。 時間が保存され、エラーの削減は別のレベルに、運用上の機能を移動することができます。

## <a name="automation-of-operational-tasks"></a>運用タスクのオートメーション
