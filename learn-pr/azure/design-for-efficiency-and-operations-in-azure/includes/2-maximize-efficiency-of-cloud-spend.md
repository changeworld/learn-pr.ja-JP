ここでは、皆さんはソリューション アーキテクトです。 所属組織である Lamna Healthcare では、ワークロードがクラウドに移行されました。 最近、これらのリソースやワークフローに対する課金が Lamna で予想した以上に増加しています。 かどうかの向上は、自然で効率的な増加や、コストを削減して、組織のクラウド リソースを効率化しているかどうかを判断するように依頼されました。
...
![オンプレミスのインフラストラクチャを使用する際の欠点を示す図。](../media/cloudcomputingpatterns.png)
...
Azure Advisor によって、使用率の低い仮想マシンが識別されます。 Advisor は 14 日間、バーチャル マシン使用率を監視しを使用率が低い仮想マシンを識別します。 仮想マシンの CPU 使用率が 5% 以下とネットワーク使用率が 7 MB、または以下の 4 つまたは複数の日が仮想マシンの使用率が低いと見なされます。
...
プールに追加できるデータベースが多ければ多いほど、節約量も多くなります。 次の図は、エラスティック データベース プールの 3 種類の機能: basic、standard、および premium です。  Basic は DB ごとに最大 5 eDTU まで、Standard は DB ごとに最大 100 eDTU まで、Premium は DB ごとに最大 1000 eDTU まで、自動的にスケールアップします。
