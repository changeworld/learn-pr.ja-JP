メッセージの送信側アプリケーションと受信側のアプリケーションを持つ図形既知のデータのパケットをキューに格納されます。 キューは送信者によって作成され、メッセージが追加されます。 受信者はメッセージを受け取り、それを処理して、キューからそのメッセージを削除します。 次の図は、このプロセスの典型的なフローを示しています。
