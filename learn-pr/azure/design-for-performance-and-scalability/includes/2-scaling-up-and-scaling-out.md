...
アプリケーションの需要が一定の世界では、必要なリソースのレベルを簡単に予測できます。 現実の世界では、アプリケーションの需要は時間と共に変化するので、必要なレベルを予測するのが困難な場合があります。 運がよければ、その変化は予測可能であったり、季節的であったりしますが、すべてのアプリケーションでそれが一般的なわけではありません。 需要に見合った適切な量のリソースをプロビジョニングし、需要の変化に応じて調整するのが理想的です。
