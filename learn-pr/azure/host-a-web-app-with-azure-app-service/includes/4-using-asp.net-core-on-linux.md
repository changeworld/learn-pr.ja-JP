Web アプリケーションをビルドするためにオープン ソース テクノロジを使用することにしました。 ASP.NET Core がクロス プラットフォームのオープン ソース フレームワークであることはわかっています。 ASP.NET Core を使用して、Linux 開発環境で web アプリを開発するには!

Azure App Service では、Node.js、PHP、または .NET Core を含む、お気に入りの web テクノロジを使用できます。
...
Web アプリケーションをビルドするようにした場合は、多くの言語とフレームワークを選択できます。 App Service では、Node.js、PHP、または .NET Core などのアプリケーションのさまざまな種類をホストすることによってこの選択を容易にするのに役立ちます。 これにより、言語と、web ホストの要件に合わせて変更する代わりに最も慣れているフレームワークを使用することができます。
