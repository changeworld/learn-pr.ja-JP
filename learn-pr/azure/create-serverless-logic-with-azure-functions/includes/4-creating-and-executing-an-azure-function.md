...
バインドは、データとサービスを作成した関数に接続するための宣言型の方法です。 バインドは、データ ソースに接続し、接続を管理するための関数でコードを記述する必要はありませんが、別のサービスと対話する方法を理解します。 プラットフォームは、その複雑さのバインド コードの一部として処理されます。 バインドごとに方向が 1 つあります。お客様のコードでは、*入力*バインディングからデータを読み取り、*出力*バインディングにデータを書き込みます。 各関数には、関数によって処理された入出力データを管理するために、0 個またはそれ以上のバインドを持つことができます。

トリガーは、特殊な種類の実行を開始する追加の機能を持つ入力バインドのです。
...
バインドは、Azure portal で定義することができ、直接編集できますも JSON ファイルとして保存されます。 次の JSON は、関数に対するトリガーとバインドの定義のサンプルです。
...
![関数エディターのブレードで、アプリ サービスのナビゲーションとファイルの表示 メニューで、選択した"HttpTriggerJS1"関数で、展開されたビュー ファイル メニューなどを示す Azure ポータルのスクリーン ショットで強調表示されます。](../media/4-file-navigation.png)

含むタブを右にフライアウト メニューが前のスクリーン ショットでわかる、**ファイルを表示**します。 このタブを選択すると、ご自分の関数を構成するファイル構造が表示されます。
...
構成されたトリガーを手動でトリガーすることによって、関数を開始できます。 たとえば、HTTP トリガーの場合 - を使用している場合ことができます使用する Postman または cURL などのツールには、HTTP トリガーの定義からは、関数エンドポイント URL、HTTP 要求を開始する (**関数の URL を取得する**)。

### <a name="testing-in-the-azure-portal"></a>Azure portal でテストする

ポータルにも、関数をテストするための便利な手段が用意されています。 コード ウィンドウの右側に、タブ付きのポップアップ ナビゲーション メニューがあります。 このメニューには、**[テスト]** 項目が含まれています。 メニューを展開してこのタブを選択して、別の方法で関数を実行し、結果を表示することができます。 このテスト ウィンドウで **[実行]** をクリックすると、その結果が出力ウィンドウに状態コードと共に表示されます。
...
![いくつかの関数の結果とモジュール メニュー項目を強調表示されている関数の対応する HTTP ステータス コードと HTTP 関数モニター ブレードを示す Azure ポータルのスクリーン ショット。](../media/4-monitor-function.png)

## <a name="streaming-log-window"></a>ストリーミング ログ ウィンドウ

Azure portal でデバッグのために、ご自分の関数にログ ステートメントを追加することもできます。 言語ごとに呼び出されるメソッドには、コード ウィンドウの下部にあるタブ付きのポップアップ メニューにあるログ ウィンドウに情報を記録するために使用できる、「ログ」オブジェクトが渡されます。
