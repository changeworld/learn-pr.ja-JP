エスカレーター駆動歯車の例を続け、温度サービスのロジックを追加しましょう。 具体的には、HTTP 要求からデータを受信します。
...
1. 初めての関数の作成を始めるには、**[関数]** をクリックし、結果のページの上部にある **[新しい関数]** ボタンをクリックします。
...
1. 次のスクリーンショットに示すように、画面に表示されたテンプレートのリストから、**[HTTP trigger]** テンプレートを選択します。

1. 表示された **[新しい関数]** ダイアログの言語フィールドを "JavaScript" に変更し、名前フィールドに「**DriveGearTemperatureService**」と入力します。 承認レベルを "Function" のままにして **[作成]** ボタンを押し、関数を作成します。

1. 関数の作成が完了すると、コード エディターが開き、*index.js* コード ファイルの内容が示されます。 テンプレートから生成された既定のコードは、次のスニペット内に一覧されます。
...
ここで追加したロジックは簡単です。 測定値の配列を反復処理して、温度フィールドを確認します。 そのフィールドの値に応じて、**[OK]**、**[CAUTION]**、または **[DANGER]** の状態を設定します。 次に、各エントリに追加された状態フィールドと共に、測定値の配列を返します。
...
1. **[実行]** を選択して、出力ウィンドウに応答を表示します。 ログ メッセージを表示するには、ページの下部にあるポップアップで **[ログ]** タブを開きます。 次のスクリーンショットでは、出力ウィンドウに応答の例と **[ログ]** ウィンドウにメッセージが示されています。
    ...
    **[モニター]** ダッシュボードに移動すると、Application Insights に要求が記録されていることを確認することができます。
