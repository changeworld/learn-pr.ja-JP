...
時間の経過と共に追加する必要がありますを実現できるコンピューティング能力に応じてが場合パフォーマンス オプションを調整したり、パフォーマンスのモデルを仮想コアと DTU のも切り替えできます。
...
1. ポータルの左上隅にある **[リソースの作成]** をクリックします。 **[Databases]** を選択し、**[SQL Database]** を選択します。
   ...
   ![データベース セクションを選択し、リソースの作成、データベース、および SQL Database のリソース ブレードの作成を示す Azure ポータルのスクリーン ショット ボタンが強調表示されます。](../media-draft/create-db.png)
...
1. フォームの残りの部分を設定するには、次の値を使用します。
    ...
    | **[リソース グループ]** |  既存のグループを使用して、 <rgn>[サンド ボックス リソース グループ名]</rgn> |
    | **ソースの選択** | **空のデータベース** |
...
プロセスが完了したら、**[ダッシュボードにピン留めする]** をクリックして、後で必要なときにすばやくアクセスできるように、データベース サーバーをダッシュボードにピン留めします。
   ...
   ![ダッシュ ボード ボタンを強調表示されている最新の展開成功のメッセージから、暗証番号 (pin) を使用して通知メニューを示す Azure ポータルのスクリーン ショット。](../media-draft/notifications-complete.png)
...
1. **[サーバー ファイアウォールの設定]** をクリックします。
    ...
    ![セット サーバー ファイアウォール ボタンが強調表示されている SQL database の概要 ブレードを表示する Azure ポータルのスクリーン ショット。](../media-draft/set-server-firewall.png)

1. **[クライアント IP の追加]**、**[保存]** の順にクリックします。

    ![SQL データベースを示す Azure ポータルのスクリーン ショットは、強調表示されている追加のクライアント IP で 設定 ブレードをファイアウォールします。](../media-draft/add-client-ip.png)
