...
Azure SQL Database のインフラストラクチャ、ソフトウェア更新プログラム、修正プログラムは Microsoft で保持しています。 Azure SQL Database のそれ以外の部分については、他の SQL Server インストールと同様に扱うことができます。 たとえば、Azure SQL database を管理するのに Visual Studio、SQL Server Management Studio、SQL Server Operations Studio、またはその他のツールを使用することができます。
...
Cloud Shell は任意の場所からアクセスできます。 ポータル以外にも、[shell.azure.com](https://shell.azure.com/)、Azure mobile app、または Visual Studio Code からも Cloud Shell にアクセスできます。 右側のパネルは、この演習の中に使用するためのターミナル クラウド シェルです。
...
ここを使用する、`az`ユーティリティ、データベースを一覧表示およびに関するいくつかの情報を表示するのには**物流**の最大サイズ、ステータスを含む、データベース。
...
1. データベース名のみが必要なので、このコマンドをもう一度実行します。 今回は、出力を `jq` にパイプして、名前フィールドのみを出力します。
    ...
    ```
    **Logistics** はデータベースです。 などの SQL Server,**マスター**アカウントでサインインし、システムの構成設定など、サーバーのメタデータが含まれています。
