...
このモジュールでは、簡略化のため CDN から直接 AngularJS を使用します。 この方法では、サーバー コンテンツで直接スクリプト ファイルの依存関係を管理するのではなく、CDN に渡します。 また、この方法では、特定のリソースで使用される CDN の速度と地理的分布に基づいて、ダウンロードの速度を改善することができます。
