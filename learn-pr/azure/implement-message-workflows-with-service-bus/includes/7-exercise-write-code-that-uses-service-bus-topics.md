Azure Service Bus のトピックを使用して、営業部門の分散アプリケーションのパフォーマンスの販売に関するメッセージを配信しようとしています。 モバイル デバイスでの販売担当者によって使用されるアプリでは、各領域と期間の売り上げ高を集計したメッセージを送信します。 これらのメッセージは、南北アメリカ、ヨーロッパなど会社の運用領域内にある web サービスに配信されます。

トピックとサブスクリプションを含む、Azure サブスクリプションで、必要なインフラストラクチャを既に実装しています。 ここで、トピックにメッセージを送信し、各サブスクリプションからメッセージを取得するコードを記述します。

## <a name="configure-a-connection-string-to-a-service-bus-namespace"></a>Service Bus 名前空間への接続文字列を構成する

送信と受信するコンポーネントの両方の接続文字列を構成することで開始します。
...
1. **デバッグ**ウィンドウの選択 ドロップダウン リストで、**起動パフォーマンスのメッセージの送信者**、およびキーを押します**F5**します。 Visual Studio Code でデバッグ モードのコンソール アプリケーションがビルドされて実行されます。
...
1. **デバッグ**ウィンドウの選択 ドロップダウン リストで、**起動パフォーマンスのメッセージの受信者**、およびキーを押します**F5**します。 Visual Studio Code でデバッグ モードのコンソール アプリケーションがビルドされて実行されます。
